...
import { useRoom, type BroadcastAnswer } from "../hooks/useRoom";
...
export default function MultiplayerSession({ initialRoom, questions, onLeave }: Props) {
  const { user } = useAuth()
  const { room, advance, finish, receivedAnswers, broadcastAnswer, resetAnswers } = useRoom(initialRoom.code)
  ...
  const bothSubmitted =
    receivedAnswers.size === participants.length &&
    Array.from(receivedAnswers.values()).every((p: BroadcastAnswer) => p.score !== null)
  ...
  useEffect(() => {
    ...
    setSubmitted(false)
    resetAnswers()
  }, [currentRoom.currentQ, resetAnswers])
  ...
  if (currentRoom.status === 'finished') {
    return (
      <div>
        ...
        {Array.from(receivedAnswers.values()).map((p: BroadcastAnswer) => (
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {participants.map((p) => (
            <span key={p.id} style={{
              ...
              borderRadius: 'var(--radius-md)',
              padding: '4px 12px',
              color: receivedAnswers.has(p.user.id) ? 'var(--color-accent)' : 'var(--color-text-secondary)',
            }}>
              @{p.user.username} {receivedAnswers.has(p.user.id) ? '✓' : '...'}
            </span>
          ))}
        </div>
      ...
      {myResult && (
        <div style={{ ...card, marginTop: '1rem' }}>
          <div style={{ marginBottom: '1rem' }}>
            <ScoreCircle score={myResult.score} />
          </div>
